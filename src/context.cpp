...
LsContext::LsContext(const Hooks::DeviceInfo& info, VkSwapchainKHR swapchain,
    ...
    if (!conf.config_file.empty()
        ...
        std::cerr << "lsfg-vk: Reloaded configuration for "
                  << name.second << ":\n";

        if (!conf.dll.empty())
            std::cerr << "  Using DLL from: " << conf.dll << '\n';

        std::cerr << "  Multiplier: " << conf.multiplier << '\n';
        std::cerr << "  Flow Scale: " << conf.flowScale << '\n';
        std::cerr << "  Performance Mode: "
                  << (conf.performance ? "Enabled" : "Disabled")
                  << '\n';

        std::cerr << "  HDR Mode: "
                  << (conf.hdr ? "Enabled" : "Disabled")
                  << '\n';

        if (conf.e_present != 2)
            std::cerr << "  ! Present Mode: "
                      << conf.e_present << '\n';
    ...
    for (size_t i = 0;
            i < static_cast<size_t>(conf.multiplier - 1);
            ++i) {
    ...
    std::cerr << "lsfg-vk: Android AHB context created (id="
              << ctxId << ")\n";
...
VkResult LsContext::present(
    ...
    // LIGHTWEIGHT SYNC
    // -------------------------------------------------------------
    // Much cheaper than vkDeviceWaitIdle()

    vkQueueWaitIdle(info.queue.second);
    ...
    VkSemaphore finalWait =
        pass.postCopySemaphores.at(
            conf.multiplier - 2
        ).handle();
    ...
    // KEEP YOUR ORIGINAL DESKTOP PATH HERE
